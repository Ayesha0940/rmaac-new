#!/usr/bin/env bash
# run_stuck_at_sweep_simple_tag_attacktarget.sh — actuator stuck_at fault
# sweep, simple_tag only, where the fault is applied to ONLY ONE role
...
# (reward split, captures, survival) as
# run_stuck_at_sweep_simple_tag_predprey.sh, minus the cross-play columns
# (which don't apply when only one role is ever attacked). See
# --attack-target in train.py's parse_args() and the "stuck_at" branch of
# __main__.
#
# Usage (run from experiments/):
#   bash run_stuck_at_sweep_simple_tag_attacktarget.sh [predator|prey] [SEED ...]
#
# Examples:
#   bash run_stuck_at_sweep_simple_tag_attacktarget.sh              # prey, seed 49 (defaults)
#   bash run_stuck_at_sweep_simple_tag_attacktarget.sh predator     # predator, seed 49
#   bash run_stuck_at_sweep_simple_tag_attacktarget.sh predator 49  # explicit seed list
...
echo "============================================================"
echo "stuck_at-attack, target=${ATTACK_TARGET} only (simple_tag only)"
...
for SEED in "${SEEDS[@]}"; do
    ...
    LOG_DIR="${SCRIPT_DIR}/logs/stuck_at_sweep_attacktarget/seed${SEED}"
    ...
    for SUFFIX in maddpg earnie rmaac m3ddpg; do
        ...
        CSV_FILE="${EXP_NAME}_stuck_at_sweep_${ATTACK_TARGET}only.csv"
        echo ""
        echo "────────────────────────────────────────────────────────────"
        ...
        mv "${EXP_NAME}_stuck_at_sweep_${ATTACK_TARGET}only.csv" "${OUT_DIR}/"
...
echo ""
echo "============================================================"
echo "stuck_at-attack, target=${ATTACK_TARGET} only, evaluation complete."
echo "Logs : ${SCRIPT_DIR}/logs/stuck_at_sweep_attacktarget/seed{${SEEDS[*]}}/"
echo "CSVs : ${SCRIPT_DIR}/../../noise_sweeps/actuation_fault/seed{${SEEDS[*]}}/predprey/*_stuck_at_sweep_${ATTACK_TARGET}only.csv"
echo "============================================================"
